...
-- Алгебричният тип Battle представя данните за дадена битка - нейното име и датата, на която се е състояла.
-- Алгебричният тип Ship представя данните за даден кораб - неговото име и годината на пускането му във вода. 
-- Алгебричният тип Outcome представя данните за резултата за даден кораб от дадена битка - името на кораба, 
-- името на битката и резултата за кораба - дали е бил повреден (damaged), потънал (sunk), или неповреден (ok).

-- Дефинирайте функция, която получава като аргумент база от данни и връща имената на всички кораби, 
-- потънали в битка, под формата на списък от двойки от вида (<име на битка>,<списък от имена на кораби, 
...
getSunkByLocation :: Database -> Name -> [Name]
getSunkByLocation (os, _, _) p = [s | (Outcome s p' r) <- os, r == "sunk" && p == p']

getSunk :: Database -> [(Name, [Name])]
getSunk (os, bs, ss) = [(p, getSunkByLocation (os, bs, ss) p) | (Battle p _) <- bs]